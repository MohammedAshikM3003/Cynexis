/*
 * CYNEXIS — 7-Servo Simultaneous Test (CH0 - CH6)
 * 
 * Hardware Target:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Active Channels: CH0, CH1, CH2, CH3, CH4, CH5, CH6
 *   - Inactive Channels: CH7..CH15 (Held OFF)
 *   - Servo Power: XL4016 = 5.00V External Servo Supply
 *   - Horn Status: DETACHED on ALL 7 Servos
 * 
 * Servo Mapping:
 *   - CH0 = Servo 1
 *   - CH1 = Servo 2
 ...
 *   START:      CH0..CH6 = 90°
 *   POSITION A: CH0..CH6 -> 60°
 *   POSITION B: CH0..CH6 -> 90°
 *   POSITION C: CH0..CH6 -> 120°
 *   POSITION D: CH0..CH6 -> 90°
 */
...
// Move all 7 active channels simultaneously in lockstep
void moveAll7Simultaneous(float startAngle, float endAngle, int stepDelayMs) {
  ...
  // Ensure exact final target angle on all 7 channels
  for (uint8_t i = 0; i < NUM_ACTIVE; i++) {
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — 7-SERVO SIMULTANEOUS TEST     ");
  Serial.println("==========================================");
  Serial.println("Active Channels: CH0..CH6 (Servos 1..7)");
  Serial.println("Inactive Channels: CH7..CH15 (OFF)");
  Serial.println("Power: XL4016 = 5.00V");
  Serial.println("Horns: DETACHED on ALL 7 servos");
  Serial.println("------------------------------------------");
  ...
  Serial.println("\n[SETUP] Initializing all 7 active servos to 90° Center...");
  for (uint8_t i = 0; i < NUM_ACTIVE; i++) {
...
void loop() {
  Serial.println("\n--- Starting 7-Servo Simultaneous Movement Cycle ---");

  // POSITION A: All move together to 60°
  Serial.println("[POSITION A] Moving CH0..CH6 together: 90° --> 60°");
  moveAll7Simultaneous(90.0, 60.0, 30);
  delay(2000);

  // POSITION B: All return together to 90°
  Serial.println("[POSITION B] Returning CH0..CH6 together: 60° --> 90°");
  moveAll7Simultaneous(60.0, 90.0, 30);
  delay(2000);

  // POSITION C: All move together to 120°
  Serial.println("[POSITION C] Moving CH0..CH6 together: 90° --> 120°");
  moveAll7Simultaneous(90.0, 120.0, 30);
  delay(2000);

  // POSITION D: All return together to 90°
  Serial.println("[POSITION D] Returning CH0..CH6 together: 120° --> 90°");
  moveAll7Simultaneous(120.0, 90.0, 30);
  delay(2000);
