/*
 * CYNEXIS — 6-Servo Simultaneous Test (CH1, CH2, CH3, CH4, CH5, CH6)
 * 
 * Hardware Target:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Active Channels: CH1, CH2, CH3, CH4, CH5, CH6 (MG996R #1..#6)
 *   - Inactive Channels: CH0, CH7..CH15 (Held OFF)
 *   - Servo Power: XL4016 = 5.00V External Servo Supply
 *   - Horn Status: DETACHED on ALL 6 Servos
 * 
 * Lockstep Simultaneous Sequence:
 *   START:      CH1..CH6 = 90°
 *   POSITION A: CH1..CH6 -> 60°
 *   POSITION B: CH1..CH6 -> 90°
 *   POSITION C: CH1..CH6 -> 120°
 *   POSITION D: CH1..CH6 -> 90°
 */
...
const uint8_t ACTIVE_CHANNELS[] = {1, 2, 3, 4, 5, 6};
const uint8_t NUM_ACTIVE = 6;
...
// Move all 6 active channels simultaneously in lockstep
void moveAll6Simultaneous(float startAngle, float endAngle, int stepDelayMs) {
  ...
  // Ensure exact final target angle on all 6 channels
  for (uint8_t i = 0; i < NUM_ACTIVE; i++) {
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — PCA9685 6-SERVO SIMULTANEOUS TEST ");
  Serial.println("==========================================");
  Serial.println("Active: CH1 CH2 CH3 CH4 CH5 CH6");
  Serial.println("Inactive: CH0 CH7..CH15 (OFF)");
  ...
  Serial.println("\n[SETUP] Initializing all 6 active servos to 90° Center...");
  for (uint8_t i = 0; i < NUM_ACTIVE; i++) {
...
void loop() {
  Serial.println("\n--- Starting 6-Servo Simultaneous Movement Cycle ---");

  // POSITION A: All move together to 60°
  Serial.println("[POSITION A] Moving CH1..CH6 together: 90° --> 60°");
  moveAll6Simultaneous(90.0, 60.0, 30);
  delay(2000);

  // POSITION B: All return together to 90°
  Serial.println("[POSITION B] Returning CH1..CH6 together: 60° --> 90°");
  moveAll6Simultaneous(60.0, 90.0, 30);
  delay(2000);

  // POSITION C: All move together to 120°
  Serial.println("[POSITION C] Moving CH1..CH6 together: 90° --> 120°");
  moveAll6Simultaneous(90.0, 120.0, 30);
  delay(2000);

  // POSITION D: All return together to 90°
  Serial.println("[POSITION D] Returning CH1..CH6 together: 120° --> 90°");
  moveAll6Simultaneous(120.0, 90.0, 30);
  delay(2000);
