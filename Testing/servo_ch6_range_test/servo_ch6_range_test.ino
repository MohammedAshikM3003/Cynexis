/*
 * CYNEXIS — PCA9685 CH6 Range Test (MG996R #6 / Gripper)
 * 
 * Hardware Target:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Target Channel: CH6 ONLY (MG996R #6)
...
#define TARGET_CH     6      // CH6 ONLY (MG996R #6)
...
void setPWM(uint8_t channel, uint16_t on, uint16_t off) {
  uint8_t reg = 0x06 + (4 * channel); // For CH6: 0x06 + 4*6 = 0x1E
  Wire.beginTransmission(PCA9685_ADDR);
  Wire.write(reg);
  Wire.write(on & 0xFF);
  Wire.write(on >> 8);
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — PCA9685 CH6 RANGE TEST (60-120°) ");
  Serial.println("==========================================");
  Serial.println("Target: MG996R #6 on PCA9685 CH6 ONLY");
  ...
  Serial.println("\n[SETUP] Initializing CH6 to 90° Center...");
  setServoAngle(90.0);
  delay(2000);
}

void loop() {
  Serial.println("\n--- Starting CH6 Range Test Cycle ---");
