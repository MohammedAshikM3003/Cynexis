/*
 * CYNEXIS — PCA9685 CH5 Range Test (MG996R #5)
 * 
 * Hardware Target:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Target Channel: CH5 ONLY (MG996R #5)
...
#define TARGET_CH     5      // CH5 ONLY (MG996R #5)
...
void setPWM(uint8_t channel, uint16_t on, uint16_t off) {
  uint8_t reg = 0x06 + (4 * channel); // For CH5: 0x06 + 4*5 = 0x1A
  Wire.beginTransmission(PCA9685_ADDR);
  Wire.write(reg);
  Wire.write(on & 0xFF);
  Wire.write(on >> 8);
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — PCA9685 CH5 RANGE TEST (60-120°) ");
  Serial.println("==========================================");
  Serial.println("Target: MG996R #5 on PCA9685 CH5 ONLY");
  ...
  Serial.println("\n[SETUP] Initializing CH5 to 90° Center...");
  setServoAngle(90.0);
  delay(2000);
}

void loop() {
  Serial.println("\n--- Starting CH5 Range Test Cycle ---");
