...
#define TARGET_CH     3      // CH3 ONLY (MG996R #3)
...
void setPWM(uint8_t channel, uint16_t on, uint16_t off) {
  uint8_t reg = 0x06 + (4 * channel); // For CH3: 0x06 + 4*3 = 0x12
  Wire.beginTransmission(PCA9685_ADDR);
  Wire.write(reg);
  Wire.write(on & 0xFF);
  Wire.write(on >> 8);
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — PCA9685 CH3 RANGE TEST (60-120°) ");
  Serial.println("==========================================");
  Serial.println("Target: MG996R #3 on PCA9685 CH3 ONLY");
  Serial.println("Power: XL4016 = 5.00V");
  Serial.println("Horn: DETACHED");
  Serial.println("Sequence: 60° -> 90° -> 120° -> 90°");
  Serial.println("------------------------------------------");
  ...
  Serial.println("\n[SETUP] Initializing CH3 to 90° Center...");
  setServoAngle(90.0);
  delay(2000);
}

void loop() {
  Serial.println("\n--- Starting CH3 Range Test Cycle ---");
