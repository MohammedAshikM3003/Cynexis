/*
 * CYNEXIS — PCA9685 CH0 Range Test (Servo 1 / MG996R)
 * 
 * Target Hardware:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Target Channel: CH0 ONLY (Servo 1)
...
#define TARGET_CH     0      // CH0 ONLY (Servo 1)
...
void setPWM(uint8_t channel, uint16_t on, uint16_t off) {
  uint8_t reg = 0x06 + (4 * channel); // For CH0: 0x06 + 4*0 = 0x06
  Wire.beginTransmission(PCA9685_ADDR);
  Wire.write(reg);
  Wire.write(on & 0xFF);
  Wire.write(on >> 8);
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println("   CYNEXIS CH0 / SERVO 1 RANGE TEST (60-120°) ");
  Serial.println("==========================================");
  Serial.println("PCA9685 Address: 0x40 (SDA=21, SCL=22)");
  Serial.println("Target: CH0 / Servo 1 ONLY");
  Serial.println("Power voltage: 5.00 V");
  ...
  Serial.println("\n[SETUP] Initializing CH0 / Servo 1 to 90° Center...");
  setServoAngle(90.0);
  delay(2000);
}

void loop() {
  Serial.println("\n--- Starting CH0 / Servo 1 Range Test Cycle ---");
