/*
 * CYNEXIS — PCA9685 CH4 Range Test (MG996R #4)
 * 
 * Hardware Target:
 *   - ESP32: SDA = GPIO 21, SCL = GPIO 22
 *   - PCA9685 Address: 0x40
 *   - Target Channel: CH4 ONLY (MG996R #4)
...
#define TARGET_CH     4      // CH4 ONLY (MG996R #4)
...
void setPWM(uint8_t channel, uint16_t on, uint16_t off) {
  uint8_t reg = 0x06 + (4 * channel); // For CH4: 0x06 + 4*4 = 0x16
  Wire.beginTransmission(PCA9685_ADDR);
  Wire.write(reg);
  Wire.write(on & 0xFF);
  Wire.write(on >> 8);
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println(" CYNEXIS — PCA9685 CH4 RANGE TEST (60-120°) ");
  Serial.println("==========================================");
  Serial.println("Target: MG996R #4 on PCA9685 CH4 ONLY");
  ...
  Serial.println("\n[SETUP] Initializing CH4 to 90° Center...");
  setServoAngle(90.0);
  delay(2000);
}

void loop() {
  Serial.println("\n--- Starting CH4 Range Test Cycle ---");
